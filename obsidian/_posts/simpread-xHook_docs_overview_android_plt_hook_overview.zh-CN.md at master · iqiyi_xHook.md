> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [github.com](https://github.com/iqiyi/xHook/blob/master/docs/overview/android_plt_hook_overview.zh-CN.md)

[](#android-plt-hook-概述)Android PLT hook 概述
===========================================
...
ELF 文件头中包含了 SHT 和 PHT 在当前 ELF 文件中的起始位置和长度。例如，libtest.so 的 SHT 起始位置为 12744，长度 40 字节；PHT 起始位置为 52，长度 32 字节。

[![[_resources/simpread-xHook_docs_overview_android_plt_hook_overview.zh-CN.md at master · iqiyi_xHook/8e4c16f41c7f5646e3541026b820b55a_MD5.png]]](https://raw.githubusercontent.com/iqiyi/xHook/master/docs/overview/res/elfheader.png)

### [](#shtsection-header-table)SHT（section header table）
...
[![[_resources/simpread-xHook_docs_overview_android_plt_hook_overview.zh-CN.md at master · iqiyi_xHook/e0351066a121356d2af7835df8f02040_MD5.png]]](https://raw.githubusercontent.com/iqiyi/xHook/master/docs/overview/res/elfpltgot.png)

### [](#phtprogram-header-table)PHT（program header table）
...
### [](#连接视图linking-view和执行视图execution-view)连接视图（Linking View）和执行视图（Execution View）
...
[![[_resources/simpread-xHook_docs_overview_android_plt_hook_overview.zh-CN.md at master · iqiyi_xHook/4f8fe2f32b73a29c089287dceb31276d_MD5.jpg]]](https://raw.githubusercontent.com/iqiyi/xHook/master/docs/overview/res/elfview.png)
...
稍微解释一下：因为 arm 处理器使用 3 级流水线，所以第一条指令取到的 `pc` 的值是当前执行的指令地址 + `8`。 于是：`dd4` + `8` + `3000` + `1b4` = `3f90`。
...
4.  从 PHT 中找到类型为 `PT_DYNAMIC` 的 segment，从中获取到 `.dynamic` section，从 `.dynamic` section 中获取其他各项 section 对应的内存地址。
...
### [](#可以直接从文件中读取-elf-信息吗)可以直接从文件中读取 ELF 信息吗？
...
### [](#目标-elf-使用的编译选项对-hook-有什么影响)目标 ELF 使用的编译选项对 hook 有什么影响？
...
### [](#hook-时遇到偶发的段错误是什么原因如何处理)hook 时遇到偶发的段错误是什么原因？如何处理？
...
*   注意：SFP 需要一个开关，让我们随时能够开启和关闭它。在 APP 开发调试阶段，SFP 应该始终被关闭，这样就不会错过由于编码失误导致的段错误，这些错误是应该被修复的；在正式上线后 SFP 应该被开启，这样能保证 APP 不会崩溃。（当然，以采样的形式部分关闭 SFP，用以观察和分析 hook 机制本身导致的崩溃，也是可以考虑的）

具体代码可以参考 `xhook` 中的实现，在源码中搜索 `siglongjmp` 和 `sigsetjmp`。

### [](#elf-内部函数之间的调用能-hook-吗)ELF 内部函数之间的调用能 hook 吗？
...
*   substrate：[http://www.cydiasubstrate.com/](http://www.cydiasubstrate.com/)
...
*   [caikelun@gmail.com](mailto:caikelun@gmail.com)
*   [https://github.com/caikelun](https://github.com/caikelun)
